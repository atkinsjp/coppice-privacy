...
//
//  Distinct notification tones for habit reminders, so a StillHabitCalmHabitTracker nudge is
...
nonisolated enum ReminderSound: String, CaseIterable, Identifiable, Sendable {
    /// StillHabitCalmHabitTracker's signature two-note whisper — the default for every habit.
    case chime
    ...
    /// habits created before this setting existed get StillHabitCalmHabitTracker's own tone.
