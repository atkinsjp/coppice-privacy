...
//
//  Distinct notification tones for habit reminders, so a CoppiceHabitsThatRest nudge is
...
nonisolated enum ReminderSound: String, CaseIterable, Identifiable, Sendable {
    /// CoppiceHabitsThatRest's signature two-note whisper — the default for every habit.
    case chime
    ...
    /// habits created before this setting existed get CoppiceHabitsThatRest's own tone.
